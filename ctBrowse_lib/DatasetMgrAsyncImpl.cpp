...
namespace ctb
{
   ...
   void DatasetMgrAsyncImpl::downloadTableAsync(TableId table_id, const CredentialWrapper& cred, TableResultCallback result_callback)
   {
   ...
   {
      auto getImageContents = std::bind_front(&DatasetMgrAsyncImpl::sndGetLabelImage, this);
      ...
      auto saveIfNeeded = [this](ImageFileContents image_contents) -> exec::task<ImageFileContents>
      {
         ...
         {
            auto [[maybe_unused]] bytes_written = co_await io_pool.sndWriteFile(image_contents.file_path, image_contents.data);
