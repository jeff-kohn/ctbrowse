...
namespace ctb
{
   ...
   auto CredentialPromptFuncWinApi::operator()(const std::string& cred_name, std::string_view prompt_message, bool /*allow_save*/)
      -> CredentialResult
   {
      ...
      try
      {
         ...
         CREDUI_INFOA cui{ .cbSize         = sizeof(CREDUI_INFO),
                           .hwndParent     = nullptr,
         ...
         cred_buf username{ { '\0' } };
         cred_buf password{ { '\0' } };
         auto     result = CredUIPromptForCredentialsA(&cui, cred_name.c_str(), nullptr, auth_error_code, username.data(),
                                                       static_cast<ULONG>(username.size()), password.data(),
                                                       static_cast<ULONG>(password.size()), &save, flags);
         if (NO_ERROR != result)
         {
