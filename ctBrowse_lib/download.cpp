...
namespace ctb
{
   namespace
   {
      // file-scope instance of HttpDownloader
      auto getDownloader() -> HttpDownloader&
      {
                                           ...
                                           uint32_t                 table_code_page) -> DownloadResult
   {
      ...
      if (convert_to_utf)
      {
         ...
         if (auto utf_text = toUTF8(table_data.data, enum_cast<TextEncoding>(table_code_page).value_or(TextEncoding::ANSI)))
         {
   ...
   void validateResponse(const HttpDownloader::HttpResult response) noexcept(false)
   {
      if (!response)
      {
   ...
   void downloadTableAsync(TableId table, const CredentialWrapper& cred, TableResultCallback callback)
   {
      ...
      getDownloader().downloadTable(table, cred, std::move(process_result));
   }
