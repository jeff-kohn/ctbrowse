...
namespace ctb::app
{
   ...
   [[nodiscard]] auto DatasetOptionsView::create(wxWindow* parent, const DatasetEventSourcePtr& source) noexcept(false)
      -> DatasetOptionsView*
   {
      if (!parent)
      {
   ...
   void DatasetOptionsView::createOptionFilters(wxStaticBoxSizer* parent)
   {
      ...
      if (dataset->getTableId() == TableId::Availability)
      {
         ...
         auto props = {
            RtdQtyDefault,   RtdQtyLinear,       RtdQtyBellCurve,    RtdQtyEarlyCurve,
            RtdQtyLateCurve, RtdQtyFastMaturing, RtdQtyEarlyAndLate, RtdQtyBottlesPerYear,
         };
         CtPropertyFilter filter{ LBL_CHECK_READY_TO_DRINK, props, FILTER_AVAILABLE_MIN_QTY,
                                  CtPropFilterPredicate{ CtPredicateType::GreaterEqual } };
         parent->Add(CheckBoxFilterCtrl::create(parent->GetStaticBox(), m_dataset_events.getSource(), filter),
      ...
      {
         // in-stock filter
         CtPropertyFilter filter{ LBL_CHECK_IN_STOCK_ONLY,
                                  { QtyOnHand },
                                  uint16_t{ 0 },
                                  CtPropFilterPredicate{ CtPredicateType::Greater } };
         parent->Add(CheckBoxFilterCtrl::create(parent->GetStaticBox(), m_dataset_events.getSource(), filter),
                     wxSizerFlags().Border(wxALL));
         m_supported_filters.insert(filter.filter_name);
      }
      ...
      if (dataset->hasProperty(PurchaseQtyRemaining))
      {
         ...
         CtPropertyFilter filter{ LBL_CHECK_WITH_REMAINING,
      ...
      if (dataset->hasProperty(CtScore) or dataset->hasProperty(MyScore))
      {
         ...
         CtPropertyFilter filter{
            LBL_CHECK_MIN_SCORE,
            { CtScore, MyScore },
            score_params.default_value,
            CtPropFilterPredicate{ CtPredicateType::GreaterEqual }
         };
         parent->Add(SpinDoubleFilterCtrl::create(parent->GetStaticBox(), m_dataset_events.getSource(), filter, score_params),
      ...
      if (dataset->hasProperty(MyPrice))
      {
         ...
         CtPropertyFilter filter{ LBL_CHECK_MIN_PRICE,
                                  { MyPrice },
                                  price_params.default_value,
                                  CtPropFilterPredicate{ CtPredicateType::GreaterEqual } };
         parent->Add(SpinDoubleFilterCtrl::create(parent->GetStaticBox(), m_dataset_events.getSource(), filter, price_params),
                     wxSizerFlags{}.Expand().Border(wxALL));
         m_supported_filters.insert(filter.filter_name);
