...
{
   /// @brief these are the fields from the de-normalized CSV that we parse and use
   ///
   /// the values map to column indices in the file, the enum is ordered to match
   /// the record class layout.
   enum class WineListProp : uint16_t
   {
      iWineID = 0,
      WineName = 13,
      Locale = 14,
