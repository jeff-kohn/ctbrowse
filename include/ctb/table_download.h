...
namespace ctb
{
   ...
   using ResultCallback = std::move_only_function<void(DownloadResult result)>;
   ...
   [[nodiscard]] auto downloadRawTableData(const CredentialWrapper& cred,
                                           ...
                                           uint32_t                 table_code_page = CP_WINDOWS_1252) -> DownloadResult;
   ...
   /// @param format   - the data format to return
   ///
   void downloadTableAsync(const CredentialWrapper& cred, ResultCallback callback, TableId table, DataFormatId format = DataFormatId::csv);
