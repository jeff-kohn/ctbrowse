...
namespace ctb
{
   ...
   struct CredPersistDisabled
   {
      ...
      auto loadCredential(std::string_view&) -> CredentialResult
      {
         return std::unexpected{
            Error{ "Credential not loaded because persistence is disabled", Error::Category::NotSupported }
         };
      }
   ...
   public:
      ...
      {
         return loadCredential(cred_name).or_else(
            [&, this](Error) -> CredentialResult
            {
               return promptCredential(cred_name, prompt_msg, allow_save);
            });
      }
