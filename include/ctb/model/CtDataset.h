...
namespace ctb
{
   ...
   public:
      ...
      [[nodiscard]] auto getDistinctValues(CtProp prop_id, std::function<bool(const PropertyMap&)> custom_filter) const
         -> PropertyValueSet override
      {
   ...
   private:
      ...
      {
         sortData();
         m_mval_filters.subscribeChanges(
            [this]
            {
               applyFilters();
            });
         m_prop_filters.subscribeChanges(
            [this]
            {
               applyFilters();
            });
      ...
      void applyFilters()
      {
         ...
         else
         {
            ...
            m_filtered_data = vws::all(m_data)
                            | vws::transform(
                                 [](auto&& rec)
                                 {
      ...
      bool applySubStringFilter(const SubStringFilter& search_filter)
      {
         ...
         m_substring_filter = {};
         applyFilters();
         auto filtered = vws::all(*m_current_view)
                       | vws::filter(search_filter)
                       | vws::transform(
                            [](auto&& rec)
                            {
                               return static_cast<const Record&>(rec);
                            })
                       | rng::to<std::vector>();
      ...
      {
         return vws::transform(*m_current_view,
                               [prop_id](const Record& row) -> const CtPropertyVal&
                               {
                                  return row[prop_id];
                               });
      }
